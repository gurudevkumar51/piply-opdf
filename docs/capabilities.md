...
- Confidence is halved when a text box touches a crop edge, flagging possible
  truncation
- Regions can be described as **ratios and relationships** — position as a
  fraction of the page, nearest neighbour in each direction, which siblings
  share an edge — so a description carries to a document nobody has seen
...
`HEADING`, a configurable OCR engine, and the layout knowledge store (K1, K2).
...
| **Layout knowledge is never written to** | The store, its schema and its CLI work and are tested, but no part of the running system calls `remember()`, so it stays empty | K5 |
| **Layout knowledge is never read for matching** | Comparing a live region against stored records is the LayoutPredictor, which does not exist | K6 |
| **Appearance hashes not populated** | `region_phash`, `hog_features` and `hu_moments` are columns and fields with nothing filling them | K3 |
| **Text knowledge has no version columns** | A row written by an older extractor is indistinguishable from a current one — the failure the layout store was built to avoid | K4 |
