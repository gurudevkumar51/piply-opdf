...
relationships, `store.py` holds them in a plain SQLite file, `provenance.py`
carries the version block. 44 tests. Two rules are enforced in code rather than
documented: only `human` and `manual_import` sources are accepted, and
`candidates()` never returns records from a different feature version.
Schema in [database.md](database.md). CLI: `piply-opdf layout-kb stats |
export | import`.

**K2 — Human actions by kind** ✅
`CONFIRMED / CORRECTED / ADDED / DELETED / REJECTED`, with `tally()` splitting
them into the two questions they answer — did we find the region, and did we
name it right. Collapsed into one counter, a detector that finds everything and
